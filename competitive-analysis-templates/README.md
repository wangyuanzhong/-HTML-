...
**思维导图页**（`type: "mindmap"`）：演示稿会在「概述」后自动插入一页；节点浮在幻灯背景上（无内层方框）。**缩放仅在编辑导图时**可调；退出编辑后保持比例且不可再调。引出说明为独立菱形节点（`callouts[]`），与树节点之间拉虚线连接，可 **+ / − 引出说明**、拖拽位置；选中引出说明后点另一树节点可改接线。编辑下另有 **+ 总节点** / **+ 分支** / **− 分支**；树节点左侧握把拖拽（`fx`/`fy`）。
