...
(function () {
  ...
  function fitTable(table) {
    ...
    var nRow = visibleRowCount(table) + 1;
    if (!nCol || !nRow) return;
    ...
    var totalCols =
      table.tHead && table.tHead.rows[0] ? table.tHead.rows[0].cells.length : nCol;
    var colsHtml = "";
    var visibleIdx = 0;
    ...
    function write(px) {
      ...
      table.style.setProperty(
        "--matrix-cell-pad-y",
        (px * PAD_Y_RATIO).toFixed(2) + "px"
      );
      table.style.setProperty(
        "--matrix-cell-pad-x",
        (px * PAD_X_RATIO).toFixed(2) + "px"
      );
  ...
  function listAllMatrixTables() {
    // legacy IDs + any future tables (#comparison-table-tN)
    return Array.prototype.slice
      .call(document.querySelectorAll('table[id^="comparison-table"]'));
  }
